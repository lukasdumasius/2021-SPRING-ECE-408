...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
  ...
  // Lukas Dumasius ECE@UIUC
  // ******* begin - CPU Convolution ******
      ...
      for(int m = 0; m < M ; m++){                      // for each output feature map
          for(int h = 0; h < H_out; h++){               // for each output value (two loops)
              for(int w = 0; w< W_out ; w++){
                  ...
                  for(int c = 0; c < C; c++){           // sum over all input channels/ input feature maps
                      for(int p = 0; p < K; p++){       // KxK filter (two loops)
                          for(int q = 0; q < K; q++){
                              y4d(b, m, h, w) += x4d(b, c, h+p, w+q) *k4d(m, c, p, q);        // calculate the multiplication of
                          }                                                                         // the convolution at this point
                      }
                  }
              }
          }
